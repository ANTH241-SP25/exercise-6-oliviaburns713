...
new_table_name <- with(NHANESdata, table(BMIcat))
prop.table(new_table_name)
prop.table(new_table_name)*100
new_data_frame <- as.data.frame(prop.table(new_table_name)*100)
ggplot(new_data_frame, aes(BMIcat, Freq)) + geom_col()
```
...
```{r}
new_table_name <- with(NHANESdata, table(Education))
prop.table(new_table_name)
prop.table(new_table_name)*100
new_data_frame <- as.data.frame(prop.table(new_table_name)*100)
ggplot(new_data_frame, aes(Education, Freq)) + geom_col()
```
...
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + labs(title="stacked bar chart", x="BMI by Gender", y="Count")

#grouped bar chart:
#ggplot(data, aes(x=Independent variable, fill=dependent variable)) + geom_bar(position = "dodge") + labs(title="", x="Gender", y="Frequency")
ggplot(NHANESdata, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "dodge") + labs(title= "grouped bar chart", x="BMI by Gender", y="Count")
```
...
ggplot(new_data_frame, aes(x=reorder(Gender, mean), y=mean)) + geom_bar(stat="identity", fill="red") + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=0.2)
labs(x="", y="", title="Standard deviation of Gender", subtitle="")
```
...
ggplot(NHANESdata, aes(x=BMI)) + geom_histogram(bins=20, color="white", fill="steelblue") + labs(title="Colored histogram with 20 bins", x="", y="")
```
...
```{r}
ggplot(NHANESdata, aes(x=DR1TSUGR, y=..density..)) + geom_histogram(bins=20, color="white", fill="purple") + scale_y_continuous(labels=scales::percent) + labs(title="Sugar Consumption", y="percent", x="Sugar Consumed")
```
...
ggplot(NHANESdata, aes(x=Race, y=DR1TPROT)) + geom_boxplot(fill="pink") + labs(x="Race", y="daily protein intake", title="daily protein intake")
```
